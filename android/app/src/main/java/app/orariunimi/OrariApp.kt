...
fun OrariApp(initialTab: Int = 0, openSavedRequest: Int = 0, openNotificationsRequest: Int = 0) {
    ...
    fun savedSelectionChanged() {
        ...
        NotificationScheduler.cancelLessonReminder(context)
        NotificationScheduler.configure(context, runNow = true)
    }
        ...
        scope.launch {
            ...
            try {
                val fresh = withContext(Dispatchers.IO) {
                    if (combined != null) {
                        if (forceRefresh) api.refreshSavedLessons(combined)
                        else api.refreshSavedLessonsIfStale(combined)
                    } else {
                        if (forceRefresh) api.refreshLessons(yearCode, requireNotNull(source))
                        else api.refreshLessonsIfStale(yearCode, requireNotNull(source))
                    }
                }
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = {
                    when {
                        examPlannerAppeal != null -> Text("Dettagli appello")
                        examPlannerOpen -> Text("I miei appelli")
                ...
                actions = {
                    val shown = calendar
                    if (shown != null && !agendaOpen && !examPlannerOpen) IconButton(onClick = {
                        agendaOpenedFromCourseDetail = false
                        agendaOpen = true
                        error = null
                    }) {
                    ...
                    if (shown?.source?.kind == SearchKind.SUBJECT && !agendaOpen && !examPlannerOpen) {
                    ...
                    } else if (!examPlannerOpen && examCourse == null && !agendaOpen && (shown?.source?.kind == SearchKind.COURSE ||
                    ...
                    } else if (!examPlannerOpen && calendar == null && courseDetail == null && examCourse == null && !agendaOpen && !settings &&
                    ...
                    calendar != null -> {
                        ...
                        CalendarScreen(shown, week, selectedDay, weekend, saved,
                            ...
                            onRefresh = { showCalendar(shown.title, shown.year, shown.source,
                                shown.combinedSubjects, forceRefresh = true) },
